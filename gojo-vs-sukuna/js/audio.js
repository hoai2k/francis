...
// ---------- soundtrack ----------
const MUSIC_MUTE = [[33.4, 34.3], [108.3, 112]];
function musicStep(s, w) {
  ...
  const hat = (v = 0.12) => nz(w, 0.03, v, { type: 'highpass', f0: 7000 });
  ...
  const pad = (ms, d, v = 0.05) => ms.forEach(m => { osc(w, 'sawtooth', mf(m), mf(m), d, v, { atk: 0.3, lp: 1200, rev: 0.5, detune: 8 }); osc(w, 'sawtooth', mf(m), mf(m), d, v, { atk: 0.3, lp: 1200, detune: -8 }); });
  const PROG = [[38, 50, 53, 57], [34, 46, 50, 53], [36, 48, 52, 55], [33, 45, 49, 52]]; // Dm Bb C A
  const ch = PROG[bar % 4];
  if (t < 11) { if (n === 0 && bar % 2 === 0) pad([50, 53, 57], 3.0, 0.035); if (n === 0 && t > 3.5) { osc(w, 'sine', mf(26), mf(26), 1.2, 0.4); } return; }
  if (t < 15) { if (n % 4 === 0) taiko(0.5 + (t - 11) * 0.12); if (t > 13.4 && n % 2 === 0) snare(0.2 + (t - 13) * 0.1); return; }
  if (t < 56) {
    if (n === 0 || n === 8 || n === 10) kick();
    if (n === 4 || n === 12) snare();
    if (t > 40 ? true : n % 2 === 0) hat(n % 4 === 2 ? 0.16 : 0.08);
    bass(n % 4 === 3 ? ch[0] + 12 : ch[0]);
    if (n === 0) pad(ch.slice(1), 1.5, 0.028);
    if (t > 40 && (n === 6 || n === 14)) osc(w, 'square', mf(ch[3] + 12), mf(ch[3] + 12), 0.12, 0.05, { lp: 3000, rev: 0.3 });
    return;
  }
  if (t < 62) { const r = t < 59 ? 4 : t < 61 ? 2 : 1; if (n % r === 0) snare(0.2 + (t - 56) * 0.07); if (n % 8 === 0) taiko(); bass(38, 0.09, 0.15); return; }
  if (t < 86) {
    if (n === 0 && bar % 2 === 0) pad([38, 45, 50, 51], 3.2, 0.04); // eerie minor 2nd
    if (n === 0 || (t > 70 && t < 76 && n === 8)) taiko(0.7);
    if (t < 70 || t > 82) { if (n % 4 === 2) osc(w, 'sine', mf([74, 77, 81, 84, 86][Math.floor(hash(s) * 5)]), 0, 0.5, 0.06, { rev: 0.7 }); }
    else { hat(0.1); if (n % 4 === 0) snare(0.3); bass(38 + (n % 8 === 7 ? 1 : 0), 0.09, 0.18); }
    return;
  }
  if (t < 92) { if (n % 4 === 0) kick(); if (n === 4 || n === 12) snare(0.6); hat(0.12); bass(n % 2 ? 50 : 38); return; }
  if (t < 108.3) {
    const big = t > 101;
    if (n === 0 || (big && n === 8)) { kick(1); taiko(0.6); }
    if (n === 8 && !big) snare(0.5); if (big && (n === 4 || n === 12)) snare(0.6);
    if (big || n % 2 === 0) hat(0.1);
    if (n === 0) pad(ch.slice(1).map(m => m + 12), 1.6, 0.04);
    bass(ch[0], 0.09, 0.2);
    if (t > 104 && n % 4 === 0) osc(w, 'square', mf(ch[3] + 24), 0, 0.2, 0.05, { rev: 0.5 });
    return;
  }
  if (t < 116.5) { if (n === 0 && bar % 2 === 0) osc(w, 'triangle', mf([62, 65, 69, 67][bar % 4]), 0, 1.5, 0.08, { rev: 0.7 }); return; }
  if (s === 1174) pad([38, 45, 50, 53, 57], 2.6, 0.06);
}
