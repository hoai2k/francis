...
Object.assign(SFX, {
  clack(w) { // the wheel turning
    nz(w, 0.08, 0.9, { f0: 2500, f1: 300 }); osc(w, 'square', 120, 60, 0.18, 0.4, { lp: 900 });
    [1, 2.4, 3.9].forEach((m, i) => osc(w + 0.02, 'triangle', 440 * m, 440 * m * 0.98, 1.4, 0.12 / (i + 1), { rev: 0.7 }));
  },
  roar(w) { osc(w, 'sawtooth', 110, 55, 1.6, 0.35, { lp: 700, atk: 0.2 }); osc(w, 'sawtooth', 116, 57, 1.6, 0.25, { lp: 700, atk: 0.2 }); nz(w, 1.6, 0.4, { type: 'bandpass', f0: 500, f1: 200, q: 1, atk: 0.2, rev: 0.4 }); },
  beam(w, v = 0.5) { osc(w, 'sawtooth', 2200, 900, 0.35, v * 0.3, { lp: 6000 }); osc(w, 'sine', 3000, 1200, 0.4, v * 0.4, { rev: 0.3 }); nz(w, 0.3, v * 0.6, { type: 'highpass', f0: 3000 }); },
  dark(w, d = 2) { osc(w, 'sine', 400, 40, d, 0.5, { atk: d * 0.3 }); nz(w, d, 0.4, { type: 'bandpass', f0: 3000, f1: 100, q: 2, atk: d * 0.3 }); },
  pressure(w, d = 1.3) { osc(w, 'sine', 45, 35, d, 0.6, { atk: d * 0.8 }); osc(w, 'sawtooth', 60, 50, d, 0.12, { atk: d * 0.8, lp: 300 }); },
  sonic(w) { nz(w, 0.4, 0.8, { type: 'highpass', f0: 1500, f1: 8000 }); osc(w, 'sine', 3000, 200, 0.3, 0.3); },
});
Object.assign(SFX, {
  vibraslap(w) { for (let i = 0; i < 14; i++) { const t = w + i * 0.045 * (1 + i * 0.05); osc(t, 'square', 900 + Math.random() * 300, 500, 0.03, 0.18 * Math.pow(0.85, i), { lp: 3000, rev: 0.3 }); nz(t, 0.02, 0.25 * Math.pow(0.85, i), { type: 'bandpass', f0: 2500, q: 3 }); } osc(w, 'triangle', 220, 180, 0.4, 0.15); },
  clap(w) { nz(w, 0.09, 0.9, { type: 'bandpass', f0: 1500, q: 1.2, rev: 0.4 }); nz(w + 0.012, 0.07, 0.6, { type: 'bandpass', f0: 1100, q: 1.5 }); },
  swap(w) { SFX.clap(w); osc(w, 'sine', 1800, 300, 0.2, 0.2); nz(w, 0.15, 0.4, { type: 'highpass', f0: 4000, f1: 8000 }); },
});
// ---------- soundtrack ----------
const MUSIC_MUTE = [[66.5, 67.4], [101, 103]];
const SEC = [[0, 'intro'], [9, 'ominous'], [18, 'fight'], [44, 'build'], [47, 'final'], [63, 'ominous'], [67.4, 'final'], [95, 'build'], [103, 'dawn']];
function musicStep(s, w) {
  const t = s * 0.1, n = s % 16, bar = Math.floor(s / 16);
  if (MUSIC_MUTE.some(([a, b]) => t >= a && t < b)) return;
  let st = 'intro'; for (const [a, v] of SEC) if (t >= a) st = v;
  ...
  const taiko = (v = 0.8) => { osc(w, 'sine', 90, 50, 0.5, v); nz(w, 0.2, v * 0.5, { f0: 600, f1: 100, rev: 0.4 }); };
  ...
  const choir = (ms, d, v = 0.035) => ms.forEach(m => osc(w, 'triangle', mf(m), mf(m), d, v, { atk: 0.5, rev: 0.8 }));
  const PROG = [[33, 45, 48, 52], [29, 41, 45, 48], [31, 43, 47, 50], [28, 40, 44, 47]]; // Am F G E
  const ch = PROG[bar % 4];
  if (st === 'intro') { if (n === 0 && bar % 2 === 0) pad([45, 52, 57], 3, 0.03); if (n === 8) osc(w, 'sine', mf(88 - (bar % 3) * 3), 0, 0.8, 0.03, { rev: 0.8 }); return; }
  if (st === 'ominous') { if (n === 0) { taiko(0.7); pad([33, 34, 40], 1.6, 0.04); } if (n === 8 && bar % 2) taiko(0.4); return; }
  if (st === 'flash') { if (n === 0 && bar % 2 === 0) choir([57, 60, 64], 3, 0.04); return; }
  if (st === 'rise') { if (n === 0) choir([ch[1] + 12, ch[2] + 12, ch[3] + 12], 1.6, 0.035); if (n % 4 === 0) bass(ch[0], 0.2, 0.12); return; }
  if (st === 'build') { const r = t % 4 < 2 ? 2 : 1; if (n % r === 0) snare(0.25); if (n % 8 === 0) taiko(); bass(33, 0.09, 0.15); return; }
  if (st === 'fight' || st === 'final') {
    if (n === 0 || n === 10) { kick(); if (st === 'final') taiko(0.5); } if (n === 4 || n === 12) snare(0.55); hat(n % 4 === 2 ? 0.14 : 0.07);
    bass(n % 4 === 3 ? ch[0] + 12 : ch[0]);
    if (n === 0) (st === 'final' ? choir : pad)(ch.slice(1).map(m => m + 12), 1.6, 0.035);
    if (st === 'final' && n % 4 === 2) osc(w, 'square', mf(ch[3] + 24), 0, 0.12, 0.04, { rev: 0.4 });
    return;
  }
  if (st === 'domain') { if (n % 4 === 0) osc(w, 'sine', mf([76, 79, 83, 86, 88][Math.floor(hash(s) * 5)]), 0, 0.6, 0.05, { rev: 0.8 }); if (n === 0) { choir([57, 64, 69], 1.6, 0.03); taiko(0.5); } return; }
  if (st === 'dawn') { if (n === 0) pad([[45, 52, 60], [41, 48, 57], [43, 50, 59], [45, 52, 60]][bar % 4], 1.6, 0.035); if (n % 4 === 0) osc(w, 'triangle', mf([69, 72, 76, 74][(n >> 2) % 4]), 0, 0.4, 0.05, { rev: 0.6 }); }
}
